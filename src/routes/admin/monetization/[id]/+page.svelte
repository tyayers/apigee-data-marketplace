  ...
  function loadProduct() {
    if (products && products.length > 0 && !product) {
      ...
      if (product?.monetizationId) {
        ...
        fetch("/api/products/" + product.id + "/monetization/" + product?.monetizationId).then((response) => {
  ...
  function submit() {
    ...
    fetch("/api/products/" + product.id + "/monetization", {
      ...
      if (result.name) {
        ...
        console.error("Monetization rate could not be created! " + JSON.stringify(result));
      }
