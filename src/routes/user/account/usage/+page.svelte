<script lang="ts">
  ...
  function updateUsageData(force: boolean = false) {
    if ((!usageData && currentUser) || force) {
      usageData = undefined;
      appUsage = undefined;
      fetch("/api/usage?email=" + currentUser?.email)
        .then((result) => {
          return result.json();
        })
        .then((data: any) => {
          ...
          fetch("/api/usage/latency?email=" + currentUser?.email)
          .then((result) => {
            return result.json();
          })
          .then((data: any) => {
            latencyData = data;
            appLatency = getAppData(data);
  ...
  function drawBasic() {
    ...
    if (latencyData) {
      let graphAppData: any[][] = [["Month"]];
      for (const [key, value] of Object.entries(appLatency)) {
        if (!graphAppData[0].includes(key)) graphAppData[0].push(key);
      ...
      var latencyChart = new google.visualization.LineChart(
        document.getElementById("chart_latency_div")
      );
      latencyChart.draw(data, latencyOptions);
...
<div class="left_menu_page">
  <div class="left_menu_page_left">
    ...
    <div class="left_menu_page_left_list">
      ...
      <a
        href="/user/account/usage"
        class="side_menu_button side_menu_button_selected"
      >
        ...
        <svg
          ...
          class="side_menu_button_logo side_menu_button_logo_selected"
          width="20px"
          viewBox="0 0 18 18"
          preserveAspectRatio="xMidYMid meet"
          focusable="false"
          ...
          ></path></svg
        >
        <span class="side_menu_button_name side_menu_button_name_selected"
          >Usage</span
        >
      </a>
  ...
  <div class="left_menu_page_right">
    <div>
      <div class="left_menu_page_right_header">
        ...
        >
          <svg
            style="margin-left: 7px; fill: var(--primary-color);"
            aria-hidden="true"
            ><path
              fill-rule="evenodd"
            ...
            {#each Object.keys(appUsage) as appName}
              <div style="text-align: center;" class="app_card">
                ...
                <div style="width: 100%; font-size: 24px;">
                  {appUsage[appName][monthNames[2]] + " calls"}<br>
                  {appLatency[appName][monthNames[2]] + " ms avg latency"}
                </div>
              </div>
            {/each}
          </div>
